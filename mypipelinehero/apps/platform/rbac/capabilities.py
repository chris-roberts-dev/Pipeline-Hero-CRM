...
CAPABILITIES: list[CapabilitySpec] = [
    ...
    CapabilitySpec("quotes.edit", "Edit draft quotes", "quotes",
                   "Applies only to DRAFT status quote versions."),
    CapabilitySpec("quotes.send", "Send quote to client", "quotes",
                   "Applies only to DRAFT status quote versions."),
    CapabilitySpec("quotes.retract", "Retract a sent quote", "quotes",
                   "The sent version remains immutable; a successor draft is created."),
    CapabilitySpec("quotes.approve", "Accept / approve a quote", "quotes",
    ...
    CapabilitySpec("quotes.line.apply_discount", "Apply discount to line or quote", "quotes"),
    CapabilitySpec("quotes.delete_draft", "Delete a draft quote version", "quotes",
                   "Hard delete. Only DRAFT versions may be deleted."),

    # --- Client Management -----------------------------------------------
    CapabilitySpec("clients.view", "View client records", "clients"),
    CapabilitySpec("clients.create", "Create new clients", "clients",
                   "Also triggered by the quote acceptance flow."),
    CapabilitySpec("clients.edit", "Edit client details", "clients"),
    CapabilitySpec("clients.merge", "Merge duplicate client records", "clients",
    ...
    CapabilitySpec("orders.edit", "Edit order notes / metadata", "orders",
                   "Commercial lines are immutable after acceptance."),
    CapabilitySpec("orders.cancel", "Cancel an order", "orders",
                   "Only permitted when no active fulfillment artifacts exist. Reason required."),
    CapabilitySpec("orders.generate_fulfillment", "Manually trigger fulfillment generation",
                   "orders", "Fulfillment artifact generation is normally system-triggered."),

    # --- Catalog Management ----------------------------------------------
    CapabilitySpec("catalog.view", "View catalog items", "catalog",
    ...
    CapabilitySpec("catalog.pricing_rules.view", "View pricing rules", "catalog",
    ...
    CapabilitySpec("workorders.update_status", "Update work order status", "workorders",
                   "Must be the assignee unless 'Full work order management' is held."),
    CapabilitySpec("workorders.manage", "Full work order management", "workorders",
                   "Includes cancel, reassign, and override."),
    CapabilitySpec("workorders.complete", "Complete work order", "workorders",
                   "Outcome notes are required to complete."),
    CapabilitySpec("workorders.view_all", "View work orders across all assignees",
                   "workorders", "Without this, users see only their own work orders."),
    ...
    CapabilitySpec("purchasing.edit", "Edit draft POs", "purchasing",
                   "Applies only to DRAFT status POs."),
    CapabilitySpec("purchasing.submit", "Submit PO to supplier", "purchasing"),
    CapabilitySpec("purchasing.receive", "Record receipt against PO lines", "purchasing"),
    CapabilitySpec("purchasing.cancel", "Cancel a PO", "purchasing",
                   "Only before supplier processing."),

    # --- Build Order Management ------------------------------------------
    CapabilitySpec("build.view", "View build orders", "build"),
    CapabilitySpec("build.manage", "Manage build orders", "build",
                   "Start, hold, resume, cancel."),
    CapabilitySpec("build.labor.record", "Record labor entries", "build"),
    CapabilitySpec("build.labor.edit_any", "Edit any user's labor entries", "build",
                   "Managers only."),
    CapabilitySpec("build.qa.review", "QA review of builds", "build",
                   "Approve or reject builds in quality review."),
    CapabilitySpec("build.cost.view", "View build cost analysis", "build",
    ...
    CapabilitySpec("billing.invoice.void", "Void an invoice", "billing",
                   "Cannot void a PAID invoice."),
    CapabilitySpec("billing.payment.record", "Record payment against invoice", "billing"),
    CapabilitySpec("billing.payment.edit", "Record payment correction or adjustment",
                   "billing",
                   "Posted payment history is never mutated in place; this creates a reversal or adjustment."),
    CapabilitySpec("billing.reports.view", "View billing reports", "billing",
    ...
    CapabilitySpec("tasks.edit", "Edit tasks", "tasks",
    ...
    CapabilitySpec("tasks.manage", "Full task management", "tasks",
                   "Includes cancel, reopen, and override. Reopen requires a reason."),

    # --- Communications --------------------------------------------------
    CapabilitySpec("communications.view", "View communication history", "communications"),
    CapabilitySpec("communications.log", "Log inbound or outbound communication",
                   "communications"),
    CapabilitySpec("communications.send", "Send supported outbound communication",
                   "communications", "Delivery may be asynchronous."),
    CapabilitySpec("communications.manage", "Edit communication metadata",
                   "communications",
    ...
    CapabilitySpec("reporting.advanced", "Access cost and margin reports", "reporting",
    ...
    CapabilitySpec("admin.capabilities.grant", "Grant per-membership capability overrides",
                   "admin"),
    CapabilitySpec("admin.org.settings", "Edit org settings", "admin",
                   "Name, logo, timezone, etc."),
    CapabilitySpec("admin.numbering.configure", "Configure entity numbering prefixes",
                   "admin"),
]
